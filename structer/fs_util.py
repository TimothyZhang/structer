...
import const
from structer.fs_manager import File


def is_object(fs_node):
    return isinstance(fs_node, File) and fs_node.file_type == const.FILE_TYPE_OBJECT


def is_filter(fs_node):
    return isinstance(fs_node, File) and fs_node.file_type == const.FILE_TYPE_FILTER
