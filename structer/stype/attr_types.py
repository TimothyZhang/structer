...
# -*- coding: utf-8 -*-
import types, copy, string, re, os
...
class ATFloat(AttrType):
    def __init__(self, min=None, max=None, default=None, **kwargs):
        ...
        if default is None:
            default = 0.0
            if self.min != None and default < self.min:
                default = self.min
            if self.max != None and default > self.max:
                default = self.max
        self._default = default        
...
def _verify_list_items(element_type, unique, unique_attrs, val, project, recurse, vlog):
    #if unique_attrs:
    #    print '>>> unique_attrs:', unique_attrs
...
class ATList(AttrType):  
    ...
    ''' 
    def __init__(self, element_type, unique=False, minlen=0, maxlen=0x7FFFFFFF, delimiter='; ', unique_attrs=[], **kwargs):
        '''
        ...
        if unique_attrs and type(element_type) is not ATStruct:
            ...
            #raise Exception('"unique_attrs" can only be applied to list of structs')
...
class ATRef(AttrType):        
    ...
    def _export(self, val, project):
        ...
        #log.warn(u'STRUCTER-12 NOT Tested yet!')
        ...
        if filter:
            for name in filter:
                if not enum.has_name(name):
                    ...
                    #raise Exception('invalid filter for enum %s: %s' % (self.name, name))
        self._filter = filter
...
#            return attr.type.get_default(project)
    ...
    def str(self, val, project):
        ...
        # templating
        st = self._str_template
        if not st:
            st = self.struct.str_template
        if st:            
            return st.safe_substitute( _StructTemplateMap(self, val, project) )
        
        # dictstr
        return unicode( dict([(attr.name, attr.type.str(val.get(attr.name), project)) for attr in self.struct.iterate()]) )
